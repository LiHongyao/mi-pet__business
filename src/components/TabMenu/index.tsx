...
const TabMenu: React.RefForwardingComponent<IRefs, IProps> = (props, ref) => {
    // props
    const {
        current = 0,
        menus,
    ...
    const [lastSelectIndex, setLastSelectIndex] = useState<number | undefined>(current);
    ...
    return (
        <div className="lhy-tabmenu" >
            {/* 标题栏 */}
            <div className="lhy-tabmenu__menus" ref={menuRef} style={{ background: menuBackgroundColor, height: menuHeight }}>
                {menus.map((menu, index) => (
                    <div
                        className={`menu-item ${selectIndex === index ? "selected" : ""}`}
                        key={index}
                        onClick={() => { _onMenuItemTap(index) }}
                    >
                        {menu}
                    </div>
                ))}
            ...
            {/* bar */}
            <div className="lhy-tabmenu_bar" style={{ height: barHeight }} ref={barRef}>{bar}</div>
            {/* 内容 */}
            <div className="lhy-tabmenu__contents" style={{ height: `${contentHeight}px` }} ref={contentRef}>
